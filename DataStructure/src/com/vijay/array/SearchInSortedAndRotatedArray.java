...
public class SearchInSortedAndRotatedArray {
	...
	public static int searchInSortedAndRotatedrray(int[] arr,int l, int h,int key){
		int mid =  (h+l)/2;
		...
		if(arr[l]<= arr[mid]){
			...
			// To check that the Key is existing in between index l to mid or not
			
			if(key>= arr[l] && key <= arr[mid])
				return searchInSortedAndRotatedrray(arr,l,mid-1, key);
			// If key doesn't exist in between l....(m-1) change the search operation form l...(m-1) to (m+1)....h.
